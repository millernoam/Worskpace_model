...
initialize <- function(){
  ...
  agentnodeatt <<- list()  #list of node attractivenesses
  happiness <<- c()     #list for happiness of each agent
  blanklist = list(0)  #a blank list
  for(i in 1:(numagents)){   # iterate over agents
    happiness <<- append(happiness, 0)     #start happiness at 0
    agentnodeatt <<- append(agentnodeatt, list(nodeatt))  #make list of lists containing the attractiveness of nodes for each agent
...
happy <- function(agent){
  if(agentstates != -10 && agentstates != 3){
